...
class Simulator:
    ...
    def run(self):
        ...
        # Round the time component of the sort key only -- actions derived as
        # start+duration can differ from another action's literal start time
        # by ~1e-15 due to float rounding, which is enough to flip a strict
        # comparison even though the two are meant to be the same instant.
        events.sort(key=lambda e: (round(e[0], 6), e[1]))
        ...
        for pa in self.plan:
            ...
            for modifier, lit in schema.conditions:
                ...
                for t, val, actor, causer in hist:
                    if pa.start + 1e-9 < t < pa.end - 1e-9:
                        self.violations.append(Violation(
                            t, pa, "over-all",
